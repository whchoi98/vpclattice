...
export InstanceClient2=$(aws ec2 describe-instances --filters 'Name=tag:Name,Values=LatticeWorkshop-InstanceClient1' 'Name=instance-state-name,Values=running' | jq -r '.Reservations[].Instances[].InstanceId')
